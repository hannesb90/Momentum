...
def main():
    logga(f"\n# Kö mot STACK_H del 2 — {datetime.now(timezone.utc).replace(microsecond=0).isoformat()}\n")
    logga(f"  {'variant':<32}{'Δ 20-26':>9}{'Δ 14-19':>9}")
    ut = {"version": "KO_MOT_STACK_H_DEL2_V1",
          "run_utc": datetime.now(timezone.utc).replace(microsecond=0).isoformat(), "C": {}}
    T = ut["C"]
    T["C1_lat_rida"] = rapport("låt vinnarna rida", dict(lat_rida=True))
    T["C3_topp5_sparr40"] = rapport("topp-5-spärr till 40", dict(stampel_spar=40))
    T["C3_topp5_sparr50"] = rapport("topp-5-spärr till 50", dict(stampel_spar=50))
    T["C4_snav_utgang10"] = rapport("snäv utgång stämplade >10", dict(stampel_exit=10))
    T["C4_snav_utgang15"] = rapport("snäv utgång stämplade >15", dict(stampel_exit=15))
    T["C5_reentry_karens3"] = rapport("återinträdesspärr 3 beslut", dict(stampel_exit=15, karens=3))
    T["C6_satellit2"] = rapport("satellitplats x2", dict(satellit=2))
    T["C7_asymmetrisk"] = rapport("asymmetrisk ombalansering", dict(asym=True))
    T["C8_time_stop13"] = rapport("time stop 13 paneler", dict(time_stop=13))
    T["C8_time_stop26"] = rapport("time stop 26 paneler", dict(time_stop=26))
    T["C10_trend05"] = rapport("trendstyrka R2 >= 0,5", dict(trend_min=0.5))
    T["C10_trend07"] = rapport("trendstyrka R2 >= 0,7", dict(trend_min=0.7))
    T["C11_korr085"] = rapport("korrelationsfyllnad 0,85", dict(korr_gr=0.85))
    T["D2_minhold2"] = rapport("min 2 beslut efter viktökning", dict(min_hold_efter_okning=2))
    T["D2_minhold3"] = rapport("min 3 beslut efter viktökning", dict(min_hold_efter_okning=3))
    (UT / "resultat_del2.json").write_text(json.dumps(ut, ensure_ascii=False, indent=1))
    n = sum(1 for v in T.values() if v["bada_positiva"])
    logga(f"\nPositiva i BÅDA fönstren: {n} av {len(T)}")
    logga(f"Skrivet: {UT/'resultat_del2.json'}")
