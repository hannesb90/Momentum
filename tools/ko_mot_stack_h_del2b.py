...
def main():
    logga(f"\n# Kö mot STACK_H del 2b, bindande trösklar — {datetime.now(timezone.utc).replace(microsecond=0).isoformat()}\n")
    ...
    T = ut["C"]
    for ts_ in (2, 3, 4, 6):
        T[f"time_stop_{ts_}p"] = rapport(f"time stop {ts_} paneler", dict(time_stop=ts_))
    for ex in (5, 8, 12, 20):
        T[f"snav_utgang_{ex}"] = rapport(f"snäv utgång stämplade >{ex}", dict(stampel_exit=ex))
    for mh in (1, 4, 6):
        T[f"minhold_{mh}"] = rapport(f"min {mh} beslut efter viktökning", dict(min_hold_efter_okning=mh))
    for sat in (1, 3, 5):
        T[f"satellit_{sat}"] = rapport(f"satellitplats x{sat}", dict(satellit=sat))
    (UT / "resultat_del2b.json").write_text(json.dumps(ut, ensure_ascii=False, indent=1))
    n = sum(1 for v in T.values() if v["bada_positiva"])
    logga(f"\nPositiva i BÅDA fönstren: {n} av {len(T)}")
    logga(f"Skrivet: {UT/'resultat_del2b.json'}")
