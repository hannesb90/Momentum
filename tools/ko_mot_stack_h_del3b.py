...
def main():
    logga(f"\n# Kö mot STACK_H del 3b, bindande trösklar — {datetime.now(timezone.utc).replace(microsecond=0).isoformat()}\n")
    logga(f"  {'variant':<34}{'Δ 20-26':>9}{'Δ 14-19':>9}")
    ut = {"version": "KO_MOT_STACK_H_DEL3B_BINDANDE_TROSKLAR", "E": {}}
    E = ut["E"]
    for dd in (0.05, 0.10, 0.15):
        E[f"E1_dd_stop_{int(dd*100)}"] = rapport(f"drawdown-stop {int(dd*100)} %", dict(dd_stop=dd))
    for dd, rk in ((0.05, 20), (0.05, 25), (0.10, 20), (0.10, 25), (0.10, 30)):
    ...
    (UT / "resultat_del3b.json").write_text(json.dumps(ut, ensure_ascii=False, indent=1))
    n = sum(1 for v in E.values() if v["bada_positiva"])
    logga(f"\nPositiva i BÅDA fönstren: {n} av {len(E)}")
    logga(f"Skrivet: {UT/'resultat_del3b.json'}")
