"""
data/data_loader.py – Hämtar och cachar veckodata primärt via Börsdata.
...
def fetch_weekly_data(
    ...
    Svenska '.ST'-tickers hämtas primärt via Börsdata Pro+. Yahoo används
    endast som explicit fallback för omatchade/misslyckade instrument.
    '.NGM'-suffix hämtas via Avanza.
    """
    ...
    price_source_contract = getattr(config, "PRICE_DATA_SOURCE", "yahoo_total_return")
    cache_key = (f"{price_source_contract}_{','.join(sorted(tickers))}_"
                 f"{start}_{resolved_end}_h{config.MIN_HISTORY_WEEKS}")
    ...
    primary_tickers = [t for t in tickers if not t.endswith(_AVANZA_SUFFIX)]
    ngm_tickers = [t for t in tickers if t.endswith(_AVANZA_SUFFIX)]

    result: Dict[str, pd.DataFrame] = {}
    use_borsdata = price_source_contract.startswith("borsdata_")
    yahoo_tickers = ([] if use_borsdata
                      else list(primary_tickers))

    if primary_tickers and use_borsdata:
        from altdata import borsdata
        instrument_map = borsdata.stockprice_instrument_map()
        split_map = borsdata.split_events_map(borsdata.stock_splits(use_cache=False))
        matched_ids = [instrument_map[t] for t in primary_tickers if t in instrument_map]
        dividend_map = {}
        for offset in range(0, len(matched_ids), 50):
            dividend_map.update(borsdata.dividend_events_map(
                matched_ids[offset:offset + 50], use_cache=False
            ))
        print(f"[DataLoader] Hämtar {len(primary_tickers)} tickers primärt från Börsdata...")
        for i, ticker in enumerate(primary_tickers, 1):
            ins_id = instrument_map.get(ticker)
            if not ins_id:
                yahoo_tickers.append(ticker)
                continue
            try:
                daily = borsdata.stockprices_ohlcv(ins_id, use_cache=True)
                daily = borsdata.adjust_ohlc_for_dividends(
                    daily, borsdata.normalize_dividends_for_splits(
                        dividend_map.get(ins_id), split_map.get(ins_id, [])
                    )
                )
                daily = daily.loc[(daily.index >= pd.Timestamp(start)) &
                                  (daily.index < pd.Timestamp(resolved_end))]
                weekly = daily.resample("W-FRI").agg({
                    "Open": "first", "High": "max", "Low": "min",
                    "Close": "last", "Volume": "sum",
                }).dropna(subset=["Open", "High", "Low", "Close"])
                weekly.index = weekly.index - pd.Timedelta(days=4)
                # Corporate actions utöver vanliga splitar (avknoppning,
                # inlösen, emission) finns inte alltid i Börsdatas splitlista.
                # En kvarvarande extrem veckorörelse gör serien olämplig som
                # target; använd då den redan totalavkastningsjusterade Yahoo-
                # serien för just detta instrument och logga källbytet.
                residual_jumps = weekly["Close"].pct_change().abs()
                if (residual_jumps > config.SUSPICIOUS_JUMP_THRESHOLD).any():
                    worst = float(residual_jumps.max())
                    print(f"  [SOURCE-FALLBACK] {ticker}: Börsdata har kvarvarande "
                          f"corporate-action-hopp {worst:.1%}; använder Yahoo.")
                    yahoo_tickers.append(ticker)
                    continue
                cleaned = _clean(weekly, ticker)
                if cleaned is not None:
                    result[ticker] = cleaned
                else:
                    yahoo_tickers.append(ticker)
            except Exception as exc:  # noqa: BLE001
                print(f"  [WARN] {ticker}: Börsdata misslyckades ({exc}); Yahoo-fallback.")
                yahoo_tickers.append(ticker)
            if i % 50 == 0:
                print(f"  [Börsdata] {i}/{len(primary_tickers)}")

    if yahoo_tickers:
        print(f"[DataLoader] Yahoo-fallback för {len(yahoo_tickers)} tickers...")
        raw = yf.download(
            yahoo_tickers,
            start=start,
            end=resolved_end,
