...
DATAKÄLLOR (sammanslagna – BörsAPI vinner där båda har data):
  ...
  · Börsdata (cache/borsdata/) – Pro+, API-cachad historik, täcker First North/
...
def _date(v):
    """Returnera ett kanoniskt ISO-datum, eller None för ogiltiga datum."""
    if not v:
        return None
    try:
    ...
    for fp in bd_cache.glob("reports_*_max20.json"):
        ...
        for row in year_rows:
            ...
            for bd_key, canon_key in BORSDATA_FIELD_MAP.items():
                raw = row.get(bd_key)
                # Datum är metadata, inte numeriska features. Tidigare gick de
                # genom float()-konverteringen och försvann, vilket tvingade
                # downstream-koden att gissa 1 maj året efter.
                val = _date(raw) if canon_key.endswith("_date") else _num(raw)
...
def metric_rows(reports: dict = None) -> list:
    """En rad per bolag/år med publiceringsdatum, F-score och momentum-mått."""
    ...
    for tk, years in reports.items():
        ...
        for i, y in enumerate(ys):
            ...
            rows.append({
                "ticker": tk, "year": y,
                # Börsdatas report_Date är dagen då rapporten publicerades.
                # Saknas den får raden inte göras synlig via en gissad dag.
                "available_date": cur.get("report_date"),
                "report_start_date": cur.get("report_start_date"),
                "report_end_date": cur.get("report_end_date"),
...
def coverage_rows(rows: list) -> list:
    """Årsvis PIT-täckning så databortfall blir synligt och versionsbart."""
    by_year = {}
    for row in rows:
        slot = by_year.setdefault(row["year"], {
            "tickers": set(), "pit_tickers": set(), "feature_cells": 0,
            "non_null_feature_cells": 0,
        })
        slot["tickers"].add(row["ticker"])
        if row.get("available_date"):
            slot["pit_tickers"].add(row["ticker"])
        for col in ("f_score", "rev_growth", "rev_accel", "margin_delta",
                    "ni_growth", "fcf_margin", "roa"):
            slot["feature_cells"] += 1
            if row.get(col) is not None:
                slot["non_null_feature_cells"] += 1
    out = []
    for year, values in sorted(by_year.items()):
        total = len(values["tickers"])
        pit = len(values["pit_tickers"])
        cells = values["feature_cells"]
        out.append({
            "year": year,
            "tickers": total,
            "pit_tickers": pit,
            "pit_coverage": round(pit / total, 4) if total else 0,
            "feature_coverage": round(
                values["non_null_feature_cells"] / cells, 4
            ) if cells else 0,
        })
    return out
...
def build():
    ...
    coverage_out = out.with_name("fundamentals_coverage.csv")
    ...
    n_pit = sum(bool(r.get("available_date")) for r in rows)
    print(f"fundamentals.csv: {len(rows)} bolagsår · {n_tk} bolag · "
          f"{n_pit / len(rows):.1%} PIT-daterade → {out}")
    print(f"täckningsdiagnostik → {coverage_out}")
