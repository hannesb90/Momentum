...
from models.lgbm_model import MomentumLGBM, walk_forward_splits  # noqa: E402
...
def test_default_embargo_is_resolved_at_call_time(monkeypatch):
    """Small växlar embargo 13→52 efter modellimporten."""
    dates = _weekly_dates(400)
    monkeypatch.setattr(config, "EMBARGO_WEEKS", 52)
    train_d, val_d, _ = walk_forward_splits(
        dates, train_weeks=200, val_weeks=100, step_weeks=50
    )[0]
    assert len(train_d) == 148
    assert len(val_d) == 48


def test_checkpoint_key_changes_with_targets_and_split_contract():
    index = _weekly_dates(4)
    base = pd.DataFrame(
        {"x": [1., 2., 3., 4.], "target_return": [0., .1, .2, .3]},
        index=index,
    )
    changed = base.copy()
    changed["target_return"] += 1.0
    model = MomentumLGBM()

    def key(df, embargo=13):
        return model._checkpoint_key(
            df, train_weeks=100, val_weeks=20, step_weeks=20,
            embargo_weeks=embargo,
        )

    assert key(base) != key(changed)
    assert key(base, embargo=13) != key(base, embargo=52)
