...
try:
    import shadow_challenger as challenger
except ImportError:
    import momentum_shadow_challenger as challenger
...
def test_stale_report_maps_to_zero_maturity_before_ranking():
    ranked = challenger.add_weekly_ranks(sample_panel())
    assert ranked.loc[2, "report_maturity"] == 0
    assert ranked.loc[2, "report_maturity_rank"] < ranked.loc[0, "report_maturity_rank"]
