...
from models.ensemble import (  # noqa: E402
    MomentumEnsemble, _topn_invested_weights, build_full_output,
    kelly_position_size,
)
...
def test_ensemble_defaults_are_resolved_at_call_time(monkeypatch):
    monkeypatch.setattr(config, "ENSEMBLE_LGBM_WEIGHT", 0.7)
    monkeypatch.setattr(config, "ENSEMBLE_LSTM_WEIGHT", 0.3)
    ens = MomentumEnsemble()
    assert ens.lgbm_w == pytest.approx(0.7)
    assert ens.lstm_w == pytest.approx(0.3)


def test_topn_default_is_resolved_at_call_time(monkeypatch):
    monkeypatch.setattr(config, "MAX_POSITIONS", 2)
    out = _topn_invested_weights({"A": 1.0, "B": 0.8, "C": 0.6})
    assert set(out) == {"A", "B"}
