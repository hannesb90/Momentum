...
def parse_args():
    ...
    p.add_argument("--lgbm-weight", type=float, default=None,
                   help="(EDGE-6-ablation) åsidosätt config.ENSEMBLE_LGBM_WEIGHT för denna körning")
    p.add_argument("--lstm-weight", type=float, default=None,
                   help="(EDGE-6-ablation) åsidosätt config.ENSEMBLE_LSTM_WEIGHT för denna körning")
...
def _assert_model_contract(model, results_dir: str) -> None:
    """Stoppa inference med modell tränad för annan horisont/features."""
    expected_horizon = int(config.FORWARD_WEEKS)
    expected_features = list(FEATURE_COLS)
    contract = getattr(model, "training_contract_", None)
    if contract is not None:
        actual_horizon = contract.get("forward_weeks")
        actual_features = contract.get("feature_cols")
        if actual_horizon != expected_horizon or actual_features != expected_features:
            raise RuntimeError(
                "Sparad LGBM-modell matchar inte aktiv segmentkonfiguration: "
                f"modell={actual_horizon}v/{len(actual_features or [])} features, "
                f"runtime={expected_horizon}v/{len(expected_features)} features. "
                "Träna om segmentet innan predict-only."
            )
        return

    stats_path = Path(results_dir) / "stats.json"
    if stats_path.exists():
        try:
            actual_horizon = int(json.loads(stats_path.read_text()).get("horizon_weeks"))
        except (TypeError, ValueError, json.JSONDecodeError):
            actual_horizon = None
        if actual_horizon is not None and actual_horizon != expected_horizon:
            raise RuntimeError(
                f"Äldre LGBM-artefakt/stats är {actual_horizon}v men aktivt "
                f"segment är {expected_horizon}v. Full omträning krävs."
            )
...
def main():
    ...
    _assert_model_contract(lgbm, config.RESULTS_DIR)
    ...
    if args.lgbm_weight is not None or args.lstm_weight is not None:
        print(f"  [EDGE-6] Ensemblevikter åsidosatta: lgbm={args.lgbm_weight} lstm={args.lstm_weight}")
        ensemble = MomentumEnsemble(
            lgbm_weight=args.lgbm_weight if args.lgbm_weight is not None else config.ENSEMBLE_LGBM_WEIGHT,
            lstm_weight=args.lstm_weight if args.lstm_weight is not None else config.ENSEMBLE_LSTM_WEIGHT,
        )
    else:
        ensemble    = MomentumEnsemble()
    ...
    if serving_pkl.exists() and holdout_start is not None:
        ...
        _assert_model_contract(serving, config.RESULTS_DIR)
        s_preds = {}
        for ticker, feat_df in model_features.items():
    ...
    # Frusen binary raw-score-challenger: separat shadow/paper-ledger, aldrig
    # konsumerad av app/produktion och får inte stoppa produktionskörningen.
    if config.ACTIVE_SEGMENT == "large":
        try:
            from models.binary_shadow import run_binary_shadow
            shadow = run_binary_shadow(model_features, config.RESULTS_DIR, start_date=holdout_start)
            if shadow is not None:
                print(f"  Binary shadow journalförd t.o.m. {shadow.index.max().date()} "
                      "(production=False).")
        except Exception as exc:
            print(f"  VARNING: binary shadow misslyckades utan att påverka produktion: {exc}")
