...
class MomentumDataset(Dataset):
    ...
    def __init__(
        ...
        self.seq_len = config.LSTM_SEQUENCE_LEN if seq_len is None else seq_len
        ...
        self.dates     = df.index
        self.n_samples = len(df) - self.seq_len
...
class MomentumLSTMNet(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        # OBS: input_size får INTE vara `len(FEATURE_COLS)` som default-argument
        # - default-argument evalueras EN GÅNG vid modulimport, innan main.py:s
        # segment-specifika drop_features-trimning (large-segmentet: 61->48
        # kolumner) hunnit köra. Måste läsas av HÄR (konstruktionstid), inte
        # vid klassdefinition, annars input.size(-1)-mismatch mot tränings-/
        # prediktionsdata (fällde nattens fulla LSTM-omträning för large).
...
class MomentumLSTM:
    ...
    def fit(
        ...
        epochs = config.LSTM_EPOCHS if epochs is None else epochs
