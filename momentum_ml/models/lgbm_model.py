...
def walk_forward_splits(
    ...
    step_weeks: Optional[int] = None,
    embargo_weeks: Optional[int] = None,
) -> List[Tuple[pd.DatetimeIndex, pd.DatetimeIndex, pd.DatetimeIndex]]:
    # Segmentvärden sätts efter modulimport i main.py. None-defaults gör att
    # Small verkligen får sitt 52v-embargo i stället för importtidens 13v.
    train_weeks = config.TRAIN_WINDOW_WEEKS if train_weeks is None else train_weeks
    val_weeks = config.VAL_WINDOW_WEEKS if val_weeks is None else val_weeks
    step_weeks = config.TEST_STEP_WEEKS if step_weeks is None else step_weeks
    embargo_weeks = config.EMBARGO_WEEKS if embargo_weeks is None else embargo_weeks
...
class MomentumLGBM:
    ...
    def __init__(self, params: dict = None):
        ...
        self.training_contract_: Optional[dict] = None
    ...
    def _checkpoint_key(
        ...
        *,
        ...
        h.update(json.dumps(list(FEATURE_COLS)).encode())
        h.update(json.dumps({
            "train_weeks": train_weeks,
            "val_weeks": val_weeks,
            "step_weeks": step_weeks,
            "embargo_weeks": embargo_weeks,
            "forward_weeks": config.FORWARD_WEEKS,
        }, sort_keys=True).encode())
        # Samma features men andra labels/horisont är en annan modellkörning.
        # Tidigare ignorerades df helt och en gammal tune-checkpoint kunde
        # därför återupptas i ett nytt experiment från samma results-katalog.
