...
def attach_fundamentals_features(
    ...
    extra säkerhetsmarginal behövs. Börsdata-exporten måste på motsvarande
    sätt innehålla ``available_date`` från API-fältet ``report_Date``.
    Årsrader utan verkligt publiceringsdatum används inte: att gissa ett
    datum kan både läcka framtida data och fördröja verklig information.
    ...
    if fund_csv_path.exists():
        try:
            ...
            if "available_date" not in fund_csv.columns:
                print("[attach_fundamentals_features] WARNING: fundamentals.csv "
                      "saknar available_date; Börsdata-features stängs av tills "
                      "'python -m altdata.fundamentals build' har körts.")
            else:
                fund_csv["available_date"] = pd.to_datetime(
                    fund_csv["available_date"], errors="coerce"
                )
                invalid = fund_csv["available_date"].isna()
                if invalid.any():
                    print("[attach_fundamentals_features] WARNING: "
                          f"{int(invalid.sum())}/{len(fund_csv)} rader saknar "
                          "verkligt publiceringsdatum och exkluderas.")
                fund_csv = fund_csv.loc[~invalid].copy()
                fund_csv["available_date"] = fund_csv["available_date"].astype(
                    "datetime64[us]"
                )
                fund_csv_by_ticker = {
                    tk: g.sort_values("available_date")
                    for tk, g in fund_csv.groupby("ticker")
                }
        except Exception as e:
    ...
    for ticker, feat in all_features.items():
        ...
        if g_csv is not None and not g_csv.empty:
            ...
            joined_csv = pd.merge_asof(
                left_csv,
                g_csv[["available_date"] + [c for c in csv_cols if c in g_csv.columns]],
                left_on="Date",
                right_on="available_date",
                direction="backward"
            )
            joined_csv = joined_csv.set_index("Date")
            if (joined_csv["available_date"].dropna() >
                    joined_csv.index.to_series().loc[
                        joined_csv["available_date"].notna()
                    ]).any():
                raise AssertionError("Börsdata point-in-time leak: framtida rapport")
            for col in csv_cols:
