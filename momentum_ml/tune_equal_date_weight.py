...
(2026-07-26/27, KORRIGERAD 2026-07-29): datum med fler noterade bolag får
automatiskt större vikt i förlustfunktionen om man tränar radvis utan
sample weights - för en TVÄRSNITTSMODELL (rangordning INOM ett datum) är
det inte självklart önskvärt.

KORRIGERING 2026-07-29, viktig omtolkning: den ursprungliga körningen
jämförde en "equal_date_weight"-variant (objective="binary" +
config.LGBM_PARAMS, MED viktning) mot "baseline" = lgbm.cls_models[i] -
men produktionens RIKTIGA fit_walk_forward (models/lgbm_model.py) har
REDAN weight=1/antal_bolag_den_veckan inbyggt sedan LambdaRank-migreringen
("Tidsutjämning (Equal Date Weighting)"-kommentaren i koden). Det fanns
alltså inget att "slå på" - frågan var redan besvarad i produktionens
favör, fast av misstag, eftersom jämförelsen dessutom blandade in en helt
annan modellfamilj (binär vs lambdarank).

Den öppna frågan är i stället den OMVÄNDA: hjälper den redan adopterade
viktningen faktiskt, eller skulle en OVIKTAD LambdaRank-modell (annars
identisk - samma hyperparametrar, relevanslabels, group=) prestera lika
bra eller bättre? "baseline" nedan = produktionens nuvarande beteende
(viktad), "unweighted" = samma uppsättning utan weight=.

Kräver att 'tune_abstention_gate.py fetch' redan körts (bara features/data
- båda varianter tränas om här, ingen gammal lgbm-modell återanvänds).
...
def main():
    ...
    print(f"[equal_date_weight] KORRIGERAD: baseline = produktionens nuvarande "
          f"(viktad) LambdaRank, 'unweighted' = samma utan weight=. {len(splits)} splits.\n")

    per_split_rows = []
    full_scores = {"baseline": [], "unweighted": []}
    last_models = {}
    ...
    for i, (train_d, val_d, test_d) in enumerate(splits):
        ...
        model_base = train_lambdarank_split(train_sub, val_d, dev_df, use_date_weight=True)
        model_unw = train_lambdarank_split(train_sub, val_d, dev_df, use_date_weight=False)
        last_models["baseline"] = model_base
        last_models["unweighted"] = model_unw

        raw_base = model_base.predict(X_te)
        raw_unw = model_unw.predict(X_te)

        for name, raw in (("baseline", raw_base), ("unweighted", raw_unw)):
        ...
        print(f"  split {i+1}/{len(splits)}: "
              ...
              f"unweighted IC={per_split_rows[-1]['test_ic']:+.3f} edge={per_split_rows[-1]['test_top_decile_edge']}")

    holdout_dates = all_dates[all_dates >= holdout_start]
    if len(holdout_dates):
        print(f"\n[equal_date_weight] Extrapolerar sista splittens modeller till holdout...")
