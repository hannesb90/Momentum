...
- [x] **4. [SCN-KÖP-2] Korrelationsfiltret slår ihop vikt i stället för att
      ...
      MAX_POSITIONS. **Steg 1 KLAR, se UTVECKLINGSLOGG #125, siffror
      SLUTGILTIGT omkörda 2026-07-31 (UTVECKLINGSLOGG #171, buggmönster
      13+14-fixen).** Ursprungsresultatet (0% saknade platser) byggde på fel
      `MAX_POSITIONS` (10 i st f large:s 15) OCH tom `SECTOR_MAP` – med rätt
      config: **20,0% av de relevanta ombalanseringstillfällena tappade en
      effektiv plats**, inte 0%. Steg 2 (påfyllnadsvariant) är alltså
      INTE längre uppenbart avfärdat – öppen fråga igen, ingen kod byggd.
- [x] **5. [EDGE-6] Fasta ensemblevikter LGBM 0,6/LSTM 0,4 aldrig
      ablation-testade** 🟠→🟢 – `models/ensemble.py:28,40-44`. **KLAR
      2026-07-31, se UTVECKLINGSLOGG #172.** Tidigare BLOCKERAD (#128, ingen
      tränad LSTM fanns) – blockeringens sanna orsak visade sig vara en
      separat bugg (#169: `MomentumLSTMNet.input_size` felaktigt vid
      importtid, kraschade all LSTM-träning för large). Fixad, LSTM tränad,
      vikterna svepta (0,5/0,5, 0,7/0,3, ren LGBM, ren LSTM). Fynd: LSTM-benet
      bär i praktiken hela holdout-robustheten (MaxDD -9,5% mot ren LGBM:s
      -21,4%) oavsett exakt vikt 30-100%; 0,7/0,3 marginellt bättre än
      produktionens 0,6/0,4 på dev, ingen skillnad på holdout. Ingen
      produktionsändring – marginalen för svag för att byta utifrån en
      155v-holdout.
